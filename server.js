...
function setProxyAuth(proxyReq, req) {
  if (SUPABASE_PUBLIC_KEY) {
    if (!req.headers.apikey) {
      proxyReq.setHeader("apikey", SUPABASE_PUBLIC_KEY);
    }
  ...
  // Keep the user-entered admin secret exactly as sent by the UI.
...
// Proxy API calls first. Keeping the same origin avoids CORS and file:// issues.
app.use(
  "/functions/v1",
...
async function serveSupabasePage(slug, res) {
  ...
  // Railway serves the content as real HTML, unlike the Supabase gateway.
  res.status(200);
