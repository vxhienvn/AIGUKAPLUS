...
const script='<script id="daily-layout-sample">(function(){const table=document.querySelector(".daily-report-table");if(!table)return;const money=n=>Math.round(Number(n||0)).toLocaleString("vi-VN")+" đ";let busy=false;function refresh(){if(busy)return;busy=true;const rows=[...table.querySelectorAll("tbody tr.daily-account-row")],dates=[...new Set(rows.map(r=>r.dataset.date))],accounts=new Set();let spend=0,messages=0,dayNo=0;rows.forEach(row=>{row.cells[0].innerHTML="";const slot=row.querySelector(".day-total-slot");if(slot)slot.innerHTML="";if(row.style.display!=="none"){spend+=Number(row.cells[4]?.dataset.spend||0);messages+=Number(row.cells[5]?.dataset.messages||0);accounts.add(row.cells[2]?.innerText.trim())}});dates.forEach(date=>{const visible=rows.filter(r=>r.dataset.date===date&&r.style.display!=="none");if(!visible.length)return;dayNo++;const sum=visible.reduce((n,r)=>n+Number(r.cells[4]?.dataset.spend||0),0);visible[0].cells[0].innerHTML="<b>"+dayNo+"</b>";const slot=visible[0].querySelector(".day-total-slot");if(slot)slot.innerHTML="<b>"+money(sum)+"</b>"});document.getElementById("daily-visible-spend").textContent=money(spend);document.getElementById("daily-visible-messages").textContent=String(messages);document.getElementById("daily-visible-accounts").textContent=String(accounts.size);busy=false}new MutationObserver(refresh).observe(table.tBodies[0],{subtree:true,attributes:true,attributeFilter:["style"]});setTimeout(refresh,0)})();</script>';
