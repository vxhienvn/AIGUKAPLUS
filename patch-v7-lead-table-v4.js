...
const marker = "AIGUKA_LEAD_TABLE_V4_TEMPLATE_EXPORT";

if (source.includes(marker)) {
  console.log("[AIGUKA] Lead table V4 already installed");
} else {
  if (!source.includes('import * as XLSX from "xlsx";')) {
    source = 'import * as XLSX from "xlsx";\n' + source;
  }

  const loaderStart = source.indexOf('async function loadUnifiedLeadReport(p, selected = "all") {');
  const pageStart = source.indexOf("async function leadsPage(req,res)", loaderStart);
  if (loaderStart < 0 || pageStart < 0) throw new Error("LEAD_V4_LOADER_ANCHOR_NOT_FOUND");

  let loader = source.slice(loaderStart, pageStart);
  const identityPattern = /    const identity = leadIdentity\(row\);\n    if \(!identity \|\| seen\.has\(identity\)\) continue;/;
  if (!identityPattern.test(loader)) throw new Error("LEAD_V4_IDENTITY_ANCHOR_NOT_FOUND");
  ...
  if (!loader.includes("    seen.add(identity);\n    paid.push(lead);")) throw new Error("LEAD_V4_SEEN_ANCHOR_NOT_FOUND");
  loader = loader.replace("    seen.add(identity);\n    paid.push(lead);", "    seen.add(rowIdentity);\n    paid.push(lead);");
  source = source.slice(0, loaderStart) + loader + source.slice(pageStart);

  const currentPageStart = source.indexOf("async function leadsPage(req,res)");
  const installStart = source.indexOf("export function installStableV7Dashboard", currentPageStart);
  if (currentPageStart < 0 || installStart < 0) throw new Error("LEAD_V4_PAGE_ANCHOR_NOT_FOUND");

  const leadPage = String.raw`// AIGUKA_LEAD_TABLE_V4_TEMPLATE_EXPORT
async function leadsPage(req,res) {
  ...
  for(const lead of leads){
    ...
    const group=groups.get(key)||{items:[],name:lead.name||"Khách hàng",phones:new Set(),hasZalo:false,maxTime:0};
  ...
  const note=report.unresolvedCount?'<div class="notice">Có '+report.unresolvedCount+' lượt quảng cáo chưa xác định đúng tài khoản QC; hệ thống không tự gán sai.</div>':'';
  ...
  const body='<div class="top"><div><h1>Khách hàng / Lead</h1><div>Một dòng cho mỗi khách × quảng cáo; các quảng cáo của cùng khách đặt liền nhau · '+esc(p.since)+' → '+esc(p.until)+'</div></div>'+exportBar+'</div>'
...
`;
  source = source.slice(0, currentPageStart) + leadPage + source.slice(installStart);
  ...
  if (exportStart < 0 || exportEnd < 0) throw new Error("LEAD_V4_EXPORT_ANCHOR_NOT_FOUND");
  const exportRoute = String.raw`  app.get('/export',async(req,res)=>{
    try{
      ...
      if(type==='leads'){
        ...
        title='BÁO CÁO NGÀY';
        headers=['Ngày','Tài khoản QC','ID tài khoản','Thẻ / Phương thức','Chi tiêu','Hội thoại Meta'];
      ...
      XLSX.utils.book_append_sheet(wb,ws,type==='leads'?'Khách hàng Lead':'Báo cáo ngày');
      ws['!cols']=headers.map((h,i)=>({wch:Math.min(45,Math.max(String(h).length+3,...rows.slice(0,300).map(r=>String(r[i]??'').length+2)))}));
      ws['!merges']=[{s:{r:0,c:0},e:{r:0,c:Math.max(headers.length-1,0)}},{s:{r:1,c:0},e:{r:1,c:Math.max(headers.length-1,0)}}];
      ws['!autofilter']={ref:XLSX.utils.encode_range({r:3,c:0},{r:Math.max(3,rows.length+3),c:headers.length-1})};
      for(let c=0;c<headers.length;c++){
        const cell=ws[XLSX.utils.encode_cell({r:3,c})];
        if(cell)cell.s={fill:{fgColor:{rgb:'DCE8F5'}},font:{bold:true,color:{rgb:'203A5F'}},alignment:{horizontal:'center',vertical:'center',wrapText:true},border:{top:{style:'thin',color:{rgb:'9EB2C8'}},bottom:{style:'thin',color:{rgb:'9EB2C8'}},left:{style:'thin',color:{rgb:'9EB2C8'}},right:{style:'thin',color:{rgb:'9EB2C8'}}}};
      }
      if(ws.A1)ws.A1.s={font:{bold:true,sz:18,color:{rgb:'203A5F'}}};
  ...
  source = source.replaceAll('"campaign / ad set","chiến dịch / nhóm quảng cáo"', '"campaign / ad set","campaign","ad set","chiến dịch / nhóm quảng cáo"');
  source = source.replaceAll("'campaign / ad set','sản phẩm'", "'campaign / ad set','campaign','ad set','sản phẩm'");

  const dailyButtonPattern = /<a class="btn green" href="\/export\?type=daily&from=([^<]+?)">Xuất CSV<\/a>/;
  if (dailyButtonPattern.test(source)) {
    source = source.replace(dailyButtonPattern, '<div class="export-actions"><a class="btn green" href="/export?type=daily&format=xlsx&from=$1">Xuất Excel</a><a class="btn" href="/export?type=daily&format=csv&from=$1">Xuất CSV</a></div>');
  }

  const css = String.raw`
/* AIGUKA_TABLE_THEME_V3 */
...
`;
  ...
  if(styleEnd<0)throw new Error("LEAD_V4_STYLE_ANCHOR_NOT_FOUND");
  source=source.slice(0,styleEnd)+css+source.slice(styleEnd);

  fs.writeFileSync(file,source,"utf8");
  const syntax=spawnSync(process.execPath,["--check",file],{encoding:"utf8"});
  if(syntax.status!==0)throw new Error(`LEAD_V4_GENERATED_SYNTAX:${syntax.stderr||syntax.stdout}`);
  console.log("[AIGUKA] Lead V4 grouped table, table theme and XLSX/CSV export installed");
}
