...
async function reconcileLeadRowsToMetaAccountTotals(baseLeads,report,p,selected) {
  ...
  for(const lead of leads){
    ...
    if(accountId&&key)counts.set(accountId,(counts.get(accountId)||0)+1);
  }
  const extra=await fetchLeadAccountCandidates(p);
  const primaryByPage=new Map();
  for(const link of extra.pageAccounts||[]){
    if(link.is_primary===true||String(link.purpose||'').toLowerCase()==='reporting'){
      if(!primaryByPage.has(String(link.page_id||''))||link.is_primary===true)primaryByPage.set(String(link.page_id||''),act(link.ad_account_id||''));
    }
  }
  const candidates=[];
  ...
  for(const candidate of candidates){
    ...
    const current=Number(counts.get(candidate.accountId)||0);
    ...
    counts.set(candidate.accountId,current+1);
  }
  return leads;
}
