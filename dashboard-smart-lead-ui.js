...
function isCombinedContactHeader(value){const key=normalized(value);return key==='sđt/zalo'||key==='sđt\/zalo'||key==='sđt-zalo'||key==='sđt,zalo'}
...
function mergeContactColumns(table){
 ...
 const keep=phone>=0?phone:zalo,remove=phone>=0&&zalo>=0?zalo:-1;
 rowsOf(table).forEach(function(row){
   const first=clean(row.cells[keep]?.innerText),second=remove>=0?clean(row.cells[remove]?.innerText):'';
   const values=[first,second].filter(function(value,index,array){return !isEmpty(value)&&array.indexOf(value)===index});
   if(row.cells[keep])row.cells[keep].innerHTML=values.length?values.map(function(value,index){return index?'<span class="aiguka-contact-secondary">Zalo: '+value.replace(/[&<>]/g,'')+'</span>':value.replace(/[&<>]/g,'')}).join(''):'-';
   if(remove>=0&&row.cells[remove])row.deleteCell(remove);
 });
 ...
 if(remove>=0&&headers[remove])headers[remove].remove();
 return keep;
}
function ensureFilterButton(th){
 let button=th.querySelector('button');
 if(!button){button=document.createElement('button');button.type='button';button.className='col-filter-btn';button.textContent='▾';th.appendChild(button)}
 button.dataset.aigukaSmartContact='1';button.title='Lọc Có SĐT/Zalo / Không có SĐT/Zalo';
 button.classList.toggle('active',contactSelection.size!==2);return button;
}
...
function updateTable(table){
 const headers=Array.from(table.querySelectorAll('thead th'));if(!headers.length)return;
 const customerIndex=headers.findIndex(function(th){return isCustomerHeader(headerText(th))});if(customerIndex<0)return;
 const contactIndex=mergeContactColumns(table);if(contactIndex<0)return;
 const updatedHeaders=Array.from(table.querySelectorAll('thead th')),customerTh=updatedHeaders[customerIndex],contactTh=updatedHeaders[contactIndex];if(!customerTh||!contactTh)return;
 ensureFilterButton(contactTh);applyContactFilter(table,contactIndex);
 const visible=rowsOf(table).filter(function(row){return getComputedStyle(row).display!=='none'}),contacts=visible.filter(function(row){return hasContact(row.cells[contactIndex]?.innerText)}).length;
 setBadge(customerTh,'customer',visible.length);setBadge(contactTh,'contact',contacts);
}
...
function showMenu(button){
 close();const menu=document.createElement('div');menu.className='aiguka-smart-contact-menu';openMenu=menu;
 menu.innerHTML='<h4>Lọc: SĐT/Zalo</h4><label><input type="checkbox" value="Có SĐT/Zalo" '+(contactSelection.has('Có SĐT/Zalo')?'checked':'')+'>Có SĐT/Zalo</label><label><input type="checkbox" value="Không có SĐT/Zalo" '+(contactSelection.has('Không có SĐT/Zalo')?'checked':'')+'>Không có SĐT/Zalo</label><div class="aiguka-smart-contact-actions"><button data-action="clear">Bỏ lọc</button><button class="primary" data-action="apply">Áp dụng</button></div>';
 document.body.appendChild(menu);const rect=button.getBoundingClientRect();menu.style.left=Math.max(8,Math.min(rect.left,innerWidth-menu.offsetWidth-8))+'px';menu.style.top=Math.min(rect.bottom+5,innerHeight-menu.offsetHeight-8)+'px';
 menu.querySelector('[data-action=clear]').onclick=function(){contactSelection=new Set(['Có SĐT/Zalo','Không có SĐT/Zalo']);close();schedule()};
 menu.querySelector('[data-action=apply]').onclick=function(){contactSelection=new Set(Array.from(menu.querySelectorAll('input:checked')).map(function(input){return input.value}));close();schedule()};
}
